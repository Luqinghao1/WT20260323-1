...
void WT_ModelWidget::initUi() {
    ...
    if (m_type <= 35) {
        if (m_type <= 23) hasInD = true;
        if (m_type <= 11) hasOutD = true;
    } else if (m_type <= 71) {
        hasInD = true;
        int sub = m_type - 36;
...
void WT_ModelWidget::onResetParameters() {
    ...
    if (ui->reDEdit)
        setInputText(ui->reDEdit, defs.value("re", 20000.0));
    if (ui->cDEdit)
        setInputText(ui->cDEdit, defs.value("C", 100.0));
    if (ui->sEdit)
        setInputText(ui->sEdit, defs.value("S", 10.0));
    if (ui->alphaEdit)
    ...
    if (ui->omga1Edit)
        setInputText(ui->omga1Edit, defs.value("omega1", 0.4));
    if (ui->omga2Edit)
        setInputText(ui->omga2Edit, defs.value("omega2", 0.08));
    if (ui->remda1Edit)
        setInputText(ui->remda1Edit, defs.value("lambda1", 1e-3));
    if (ui->remda2Edit)
        setInputText(ui->remda2Edit, defs.value("lambda2", 1e-4));
...
void WT_ModelWidget::onExportData() {
    if (res_tD.isEmpty()) {
        QMessageBox::warning(this, "提示", "无曲线数据！");
        return;
    }
